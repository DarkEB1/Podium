...
2. **The marketplace** — lead with the goods. Built in TWO variants (decision 2026-08-05); both ship to staging behind a switch (`?market=skyline|rally`, default `skyline`) and Nicholas picks the winner in implementation review, after which the loser is deleted:
   ...
   Replaces `marketplace-preview.tsx` + `social-proof.tsx`. Amended 2026-08-05 (EB1 QA session, commit 7aad969): no testimonial chips and no "live" claim — fabricated social proof on a pre-launch product is a misleading commercial practice. Heading is "The marketplace"; an honest caption ("Illustration of how discovery works...") sits under the selector. The hero additionally carries a wordmark-home link and a Sign in link (top strip).
