...
/**
 * The admin 2FA login challenge: verify a live TOTP code or a one-time recovery
 * code and, on success, mark this session 2FA-passed (2.4). Admin only, tightly
 * rate-limited because a 6-digit code is otherwise brute-forceable.
 */
export async function POST(request: NextRequest) {
  ...
  const limited = await consume(userKey('admin_2fa_verify', user.id), RATE_LIMITS.admin2faByUser)
