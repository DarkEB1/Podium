-- ============================================================
-- Extend erase_user_data() to also scrub the Athlete's per-deal payment
-- details captured at signing (P2P), added in
-- 20260911000002_contract_signatures_payment_details.sql.
--
-- This create-or-replace reproduces the ENTIRE function body from
-- 20260907000004_erase_user_data_esign.sql UNCHANGED, with ONE addition:
-- contract_signatures.payment_details is nulled inside the existing signature
-- anonymisation block (Block A), alongside signer_ip / signer_device /
-- signature_image_path / typed_name. Bank/account details are personal
-- financial data with no tamper-evidence value, so they must not survive an
-- Art. 17 erasure. signed_at and signature_hash remain untouched (they are the
-- tamper-evidence for the retained, anonymised contract). No other statement is
