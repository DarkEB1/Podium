...
const PUBLIC_PATHS = [
  ...
  // WS-INFRA-01: SEO / discovery metadata. These are generated by app/robots.ts,
  // app/sitemap.ts, app/opengraph-image.tsx, app/twitter-image.tsx and
  // app/manifest.ts, and are crawled with no session. The matcher does not
  // exclude them, so without these entries every one 307'd to /auth — Google
  // read that as "no robots.txt", the sitemap was undiscoverable and link
  // previews had no image.
  '/robots.txt',
  '/sitemap.xml',
  '/manifest.webmanifest',
  '/opengraph-image',
  '/twitter-image',
  // WS-INFRA P2: the branded 403 must render for signed-out visitors too, not
  // bounce to the auth wall. robots.ts still keeps it out of the index.
  ROUTES.forbidden,
  ROUTES.auth.signIn,
  '/auth/callback',
  '/auth/confirm',
...
/**
 * WS-INFRA P2 — the authenticated surfaces a signed-out visitor is bounced to
 * sign-in from. Everything NOT in this list (and not public) falls through so
 * Next renders the branded 404 instead of the deny-by-default `/auth` redirect
 * that hid the 404 page from every anonymous visitor and returned a soft 200 to
 * crawlers for garbage URLs. This is an exhaustive allow-list of the real
 * signed-in pages: the four role areas, the shared dashboard/settings, and the
 * two auth-flow pages that require a session. `/api/*` is handled separately
 * (JSON 401, WS-SEC-05) above this check; RLS and each page's own guard remain
 * the actual access control — this only decides redirect-vs-404 for the shell.
 */
...
export async function middleware(request: NextRequest) {
  ...
  /**
   * WS-INFRA P2 — sign-in redirect that remembers where the visitor was headed.
   * The attempted path (with its query) rides along as `?next=` so login-form
   * can return the user there instead of the generic dashboard. Only the
   * same-origin path is stored, and login-form re-validates it before use, so
   * this cannot be coerced into an open redirect.
   */
  function redirectToSignIn() {
    const redirectUrl = request.nextUrl.clone()
    const attempted = redirectUrl.pathname + redirectUrl.search
    redirectUrl.pathname = ROUTES.auth.signIn
    redirectUrl.search = ''
    if (attempted.startsWith('/') && !attempted.startsWith('//')) {
      redirectUrl.searchParams.set('next', attempted)
    }
    return withCookies(NextResponse.redirect(redirectUrl))
  }
  ...
  if (!user && !isPublic) {
    ...
    // WS-INFRA P2: only genuinely private areas send a signed-out visitor to
    // the auth wall (carrying a ?next= return-to). Every other unknown path
    // falls through so Next renders the branded 404/403 instead of a soft-200
    // sign-in page — the deny-by-default 307 hid the 404 from anonymous
    // visitors and gave crawlers a soft 200 for garbage URLs.
    if (matches(pathname, PRIVATE_PREFIXES)) {
